...
test:
	mkdir -p $(BUILD_DIR)
	$(CXX) $(CXXFLAGS) $(TEST) $(LDFLAGS) -o $(BUILD_DIR)/list
...
valgrind:
	mkdir -p $(VAL_DIR)
	$(CXX) $(CXXFLAGS) $(TEST) $(VAFLAGS) -o $(VAL_DIR)/list
	$(VALGRIND) $(VALGRINDFLAGS) ./$(VAL_DIR)/list

run:
	./$(BUILD_DIR)/list

coverage: $(BUILD_DIR)/list
	mkdir -p $(COV_DIR)
	./$(BUILD_DIR)/list
